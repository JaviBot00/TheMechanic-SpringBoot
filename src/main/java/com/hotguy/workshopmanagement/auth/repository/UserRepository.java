 ...
 *
 * <p>
 * Spring Data genera automáticamente la implementación SQL de cada método
 * basándose en su nombre. No es necesario escribir ninguna query.
 *
 * <p>
 * Ejemplos de derivación de queries por nombre:
 * <ul>
 * <li>{@code findByUsername} →
 * {@code SELECT * FROM users WHERE username = ?}</li>
 * <li>{@code existsByUsername} →
 * {@code SELECT COUNT(*) > 0 FROM users WHERE username = ?}</li>
 * </ul>
 */
@Repository
