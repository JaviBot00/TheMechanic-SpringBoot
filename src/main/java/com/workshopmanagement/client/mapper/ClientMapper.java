package com.workshopmanagement.client.mapper;

import com.workshopmanagement.client.dto.ClientRequest;
import com.workshopmanagement.client.dto.ClientResponse;
import com.workshopmanagement.client.model.Client;
import org.mapstruct.*;

/**
 * Mapper MapStruct para conversión entre la entidad {@link Client} y sus DTOs.
 *
 * <p>MapStruct genera en tiempo de compilación el código de conversión entre
 * objetos. Es más eficiente que frameworks de reflection como ModelMapper porque
 * el código generado es Java plano, sin reflection en runtime.
 *
 * <p>{@code componentModel = "spring"}: el mapper generado es un bean de Spring
 * ({@code @Component}), inyectable con {@code @Autowired} o {@code @RequiredArgsConstructor}.
 * Este comportamiento se configura globalmente en el {@code pom.xml} con
 * {@code -Amapstruct.defaultComponentModel=spring}, por lo que podría omitirse aquí,
 * pero lo dejamos explícito por claridad.
 *
 * <p>{@code unmappedTargetPolicy = ERROR}: si hay campos del destino que no se mapean,
...
public interface ClientMapper {
     ...
     *
     * <p>{@code @Mapping(target = "id", ignore = true)}: el ID lo genera la BD,
     ...
     *
     * <p>{@code expression}: cuando el campo del destino no existe directamente
     ...
     *
     * <p>{@code @MappingTarget}: indica que este parámetro es el objeto destino
     * que se va a modificar en lugar de crear uno nuevo. MapStruct solo actualiza
     * los campos mapeados, dejando los demás intactos (ej. {@code id}, {@code createdAt}).
     *
     * <p>{@code NullValuePropertyMappingStrategy.IGNORE}: si un campo del DTO es null,
     * no sobreescribe el valor actual de la entidad. Útil para actualizaciones parciales.
     *
